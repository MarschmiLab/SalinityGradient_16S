---
title: "04B Phylogenetic Tree Inspection & Rooting"
author: "sophiaaredas"
date: "`r Sys.Date()`"
...
## Root tree from Archaea
If you do not have Archaea in your dataset, please skip this section and go to line 232 so that you can midroot your tree
```{r root-archaea}
...
# Root the tree using node 3394
archaea_root_tree <- ape::root(unrooted_tree, node = 3394, resolve.root = TRUE)

# Check if tree is rooted
is.rooted(archaea_root_tree) # should be TRUE

# plot unrooted tree with node 3394 highlighted
archaea_tree <- 
  ggtree(unrooted_physeq) +
  geom_tippoint(mapping = aes(color=Kingdom),
                size = 1.5) +
...
archaea_rooted_text
```
First we loaded in our unrooted tree. Because we have Archaea we will want to root from there. 
