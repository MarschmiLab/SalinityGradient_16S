...
kruskal_stations_df <- 
  obs_div_df %>%
  # Group by hill number
  group_by(Order.q) %>%
  # Run a kruskall wallis test on the hill number by station
  summarize(p.value = kruskal.test(qD ~ station_names)$p.value)
```
...
```{r plot-richness-sal, fig.height=4, fig.width=4}
richness_df %>% 
  ggplot(aes(x = salinity_psu, y = qD)) + 
  geom_point(aes(color = station)) + 
