...
for FILE in `ls /workdir/in_class_data/SalinityGradient_16S/raw_gzipped_fastqs/*.fastq.gz`
  do
  ln -s $FILE /workdir/mls528/git_repos/BIOMI6300_16S_Amplicon_Lesson/data/01_DADA2/01_raw_gzipped_fastqs/
  done
```
...
pacman::p_load(tidyverse, devtools, dada2, phyloseq, patchwork, DT, # Load ShortRead package for DNA manipulation
               install = FALSE)
```
...
```{r track_reads, fig.width=4.5, fig.height=4}
...
5. 
5. `track_read_counts.RData`: To track how many reads we lost throughout our workflow that could be used and plotted later. We will add this to the metadata in `analysis/02_Taxonomic_Assignment.Rmd`.   
